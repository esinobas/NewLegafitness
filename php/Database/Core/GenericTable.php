...
class GenericTable implements TableIf {
   ...
   */
    ...
    * It is protected to avoid it is instanced
    */
   ...
   /**
    * Open the table.
    * Load the table date from database into memory
    */
   ...
   public function delete() {
      ...
      // $this->refresh();
      $this->loggerM->debug("Delete result [ ".($result?"Success":"Fail"). " ]");
      $this->loggerM->trace ( "Exit" );
      return $result;
   }
   /**
    * Inserts data in the table.
    * The data are passed in an array parameter,
   ...
   public function insertData($theDataArray) {
      $this->loggerM->trace ( "Enter" );
      $newId = - 1;
      if (DatabaseMgr::insert ( $this->tableMappingM, $theDataArray, $this->tableDataM, $this->tableDefinitionM->getKeys ()[0], $newId )) {
         $this->loggerM->trace ( "The data was inserted successfully. " . "The new Id is [ $newId ]" );
         $this->strErrorM = "";
      } else {
         $this->strErrorM = DatabaseMgr::getDatabaseError ();
         $this->loggerM->error ( "The data was not inserted [ " . $this->strErrorM . " ]" );
      }
      $this->rewind ();
   ...
   public function update() {
      ...
      if ($result) {
         ...
         $this->strErrorM = DatabaseMgr::getDatabaseError ();
         $this->loggerM->error ( "The row was not updated [ " . $this->strErrorM . " ]" );
      }
      $this->loggerM->trace ( "Exit" );
      return $result;
   }
   ...
   public function updateRow() {
      ...
      $result = DatabaseMgr::updateTable ( $this->tableMappingM, array (
            current ( $this->tableDataM )
      ) );
      if ($result) {
         // $this->mergeTableData();
         $this->strErrorM = "";
   ...
   protected function get($theColumn) {
      $this->loggerM->trace ( "Enter" );
      $this->loggerM->debug ( "Get value from column [ $theColumn ]->" . "[ " . current ( $this->tableDataM )[$theColumn] . " ]" );
   ...
   protected function set($theColumn, $theValue) {
      $this->loggerM->trace ( "Enter" );
      $this->loggerM->trace ( "Current value in [ $theColumn ] -> [ " . current ( $this->tableDataM )[$theColumn] . " ]" );
   ...
   public function searchByColumn($theColumn, $theValue) {
      ...
      $this->loggerM->debug ( "Searching value [ $theValue ] in column " . "[ $theColumn ]" );
   ...
   public function skip($theNumRows) {
      ...
      if ($theNumRows >= $this->getCardinality ()) {
         $this->loggerM->warn ( "The skipped [ $theNumRows ] is greater then or equal the tables rows [ " . $this->getCardinality () . " ]" );
      }
      if ($theNumRows == 0) {
