...
require_once 'PhisicalTableDef.php';
class TableMapping {
   ...
   public function addTable($theTable) {
      $this->loggerM->trace ( "Enter" );
      $this->loggerM->trace ( "Add table [ $theTable ]. Num. tables [ " . count ( $this->phisicalTablesM ) . " ]" );
    ...
    *           The phisical column name
    * @param string $theLogicalColumn:
    *           The logical column name
    * @param integer $theDataType:
    *           The phisical column data type.
    */
   public function addColumn($theTable, $thePhisicalColumn, $theLogicalColumn, $theDataType) {
      $this->loggerM->trace ( "Enter" );
      
      $composedColumn = $thePhisicalColumn;
      $this->loggerM->trace ( "Add column [ $thePhisicalColumn ] in [ $theTable ]. " . "With data type [ $theDataType ]. Num. colums [ " . count ( $this->phisicalTablesM [$theTable] ) . " ]" );
      // Should be check if the parameter $the table exist before
      // inserte the mapping
      $this->phisicalTablesM [$theTable]->addColumn ( $thePhisicalColumn, $theDataType, $theLogicalColumn );
      $this->loggerM->trace ( "Exit" );
   }
   ...
   public function addCondition($theCondition) {
      $this->loggerM->trace ( "Enter" );
      $this->loggerM->trace ( "Add condition [ $theCondition ]. Num. conditions [ " . count ( $this->conditionsM ) . " ]" );
   ...
   public function getColumns($theTable) {
      $this->loggerM->trace ( "Exit" );
      $this->loggerM->trace ( "Get columns of the table [ " . $theTable . " ] that has [ " . count ( $this->phisicalTablesM [$theTable]->getColumns () ) . " ]" );
      $this->loggerM->trace ( "Exit" );
    ...
    *
    * @param
    *           theOrderBy The clausule order by
    */
