 ...
 * }
 */
require_once 'php/Database/Type/MySqlDatabase.php';
...
class DatabaseMgr {
   ...
   const DatabaseConfigC = "php/Database/Type/Database.ini";
   const modifiedRowC = "ColumnModifiedRow";
   const ORDER_BY_COLUMN_C = 'column';
   const ORDER_BY_TYPE_C = "type";
   private static $databaseM = null;
   private static $dataBaseErrorM = "";
   ...
   static private function getDatabase($theAutoCommit = true) {
      ...
      if (self::$databaseM == null) {
         ...
         if (self::$databaseM->connect ( $theAutoCommit )) {
            ...
            $logger->error ( "An error has been produced in connect with database. Error [ " . self::$dataBaseErrorM . "] " );
   ...
   static private function createSqlSelect(TableMapping $theTableMapping) {
      ...
      $logger->trace ( "Get conditions from table mapping. There is/are [ " . count ( $theTableMapping->getConditions () ) . " ] conditions" );
      $isFirst = true;
      ...
      $logger->trace ( "Get clausule order by from the table mapping. There is " . (count ( $theTableMapping->getOrderBy () ) == 0 ? "not" : "") . " clausule order by" );
         ...
         for($idx = 0; $idx < count ( $resultQuery ); $idx ++) {
            ...
            for($i = 0; $i < count ( $tableNames ); $i ++) {
               ...
               for($idxKeys = 0; $idxKeys < count ( $keys ); $idxKeys ++) {
                  $logger->trace ( "Get value for row [ $idx ] key [ $tableNames[$i]_$keys[$idxKeys] ]" . " -> [ " . $resultQuery [$idx] [$tableNames [$i] . '_' . $keys [$idxKeys]] . " ]" );

                  $theReturnData [$idx] [$theTableMapping->getColumns ( $tableNames [$i] )[$keys [$idxKeys]]] = $resultQuery [$idx] [$tableNames [$i] . '_' . $keys [$idxKeys]];
               }
            }
    ...
    *
    * @param $theTableDef. Class
    *           PhisicalTableDef where is saved the
    *           phisical table definition
    * @return string with the sql update stament
    */
   static protected function createSqlUpdate(PhisicalTableDef $theTableDef, array $theRowData) {
      $logger = LoggerMgr::Instance ()->getLogger ( __CLASS__ );
      $logger->trace ( "Enter" );
      ...
      for($i = 0; $i < count ( $theRowData ); $i ++) {
         ...
         if ($phisicalColumn) {
            if ($isFirst) {
               ...
               if ($theTableDef->getDataType ( $phisicalColumn ) == ColumnType::stringC || $theTableDef->getDataType ( $phisicalColumn ) == ColumnType::timestampC) {
                  $sqlUpdate .= "'" . $value . "'";
               } else {
                  $sqlUpdate .= $value;
               }
    ...
    */
   static public function updateTable(TableMapping $theTableMapping, array $theTableData) {
      $logger = LoggerMgr::Instance ()->getLogger ( __CLASS__ );
      $result = true;
      $logger->trace ( "Enter" );
      $logger->trace ( "Filter the modified rows. The table has [ " . count ( $theTableData ) . " ] rows before the filter" );
      ...
      $logger->trace ( "The table has been filter. The table has [ " . count ( $arrayModifiedRows ) . " ] rows after the filter" );
      ...
      if ($database != null) {
         ...
         for($i = 0; $i < count ( $arrayModifiedRows ); $i ++) {
            for($x = 0; $x < count ( $tables ); $x ++) {
               $sqlStament = self::createSqlUpdate ( $theTableMapping->getTableDefinition ( current ( $tables )->getName () ), current ( $arrayModifiedRows ) );
               next ( $tables );
            }
            reset ( $tables );
            ...
            if ($database->sqlCommand ( $sqlStament ) == 0) {
               ...
               $logger->error ( "A error has produced [ " . $database->getSqlError () . " ]" );
               $error = true;
               $result = false;
    ...
    * @return string
    */
   static protected function createSqlDelete(PhisicalTableDef $thePhisicalTableDef, array $theRowData) {
      $logger = LoggerMgr::Instance ()->getLogger ( __CLASS__ );
      $logger->trace ( "Enter" );
      $sqlDelete = "delete from " . $thePhisicalTableDef->getName () . " where ";
      $sqlDelete .= $thePhisicalTableDef->getKey () . " = ";
      if ($thePhisicalTableDef->getDataType ( $thePhisicalTableDef->getKey () ) == ColumnType::stringC) {
    ...
    * @param array $theRow
    */
   static public function delete(TableMapping $theTableMapping, array $theRow) {
      $logger = LoggerMgr::Instance ()->getLogger ( __CLASS__ );
      $logger->trace ( "Enter" );
      $result = true;
      $database = self::getDatabase ();
      if ($database->connect ( false )) {
         ...
         foreach ( $theTableMapping->getTables () as $table ) {
            if ($table->getKey () != null) {
               ...
               $sqlDelete = self::createSqlDelete ( $table, $theRow );
               ...
               if ($database->sqlCommand ( $sqlDelete ) == 0) {
                  ...
                  $logger->error ( "The command [ $sqlInsert] fails. Error [ " . $database->getSqlError () . " ]" );
                  $result = false;
                  self::$dataBaseErrorM = $database->getSqlError ();
                  break;
               }
            } else {
               $logger->trace ( "The table [ " . $table->getName () . " ] has not key. Skip the table" );
            }
         }
         if ($result) {
            $database->commit ();
   ...
   static protected function createSqlInsert(PhisicalTableDef $theTableDef, array $theData) {
      $logger = LoggerMgr::Instance ()->getLogger ( __CLASS__ );
      $logger->trace ( "Enter" );
    ...
    * @param
    *           Integer [in|out] $theNewId: It is the id of the new row inserted in
    *           into the table in the database.
    * @return boolean
    */
   static public function insert(TableMapping $theTableMapping, array $theNewData, array &$theReturnData, $theColumnKey, &$theNewId) {
      $logger = LoggerMgr::Instance ()->getLogger ( __CLASS__ );
      $logger->trace ( "Enter" );
      $result = true;
      $database = self::getDatabase ( false );
         ...
         foreach ( $theTableMapping->getTables () as $table ) {
            if ($table->getKey () != null) {
               ...
               if ($database->sqlCommand ( $sqlInsert ) == 0) {
                  ...
                  $logger->trace ( "The column key [ $theColumnKey ] will be " . "set to [ " . $database->getLastId () . " ]" );
